...
--     inner join agabang_dw.retooldb_shop_info as B
--         on B.shop_cd = A.shop_cd
    ...
    AND case when {{ select_main_type.value === 'shop' }} then A.shop_cd = '{{ select_sub_type.value }}'
             when {{ select_main_type.value === 'channel' }} then B.channel_gb = '{{ select_sub_type.value }}'
             else true end 
...
final AS (
    SELECT
        ...
        toDate(B.fin_date) AS fin_date,
        toDate(B.fsale_date) AS fsale_date,
        B.tot_in_qty AS tot_in_qty,
        B.tot_rt_in_qty AS tot_rt_in_qty,
        B.tot_sales_qty AS tot_sales_qty,
        B.tot_cust_rqty AS tot_cust_rqty,
        B.tot_wh_rtn_qty AS tot_wh_rtn_qty,
        B.tot_rt_rtn_qty AS tot_rt_rtn_qty,
        B.tot_adjt_qty AS tot_adjt_qty,
        B.tot_stock_qty AS tot_stock_qty
