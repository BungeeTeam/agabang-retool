...
with
   ...
   select
    ...
    and ( ( coalesce(C.item_md_category_id, 91) = {{sel_category.value}} ) or ( {{sel_category.value}} = 89 ) )
    and A.out_qty > 0
    group by 1,2
),
    ...
    JOIN FIRST_OUT_DAY as E
        on concat(B.year_nm_short, B.sesn_nm_eng) = E.year_season and
           case when {{sel_category.value}} = 89 then 89 else  coalesce(C.item_md_category_id, 91) end = E.cat_id
...
DAY_DATA as (
    select concat(B.year_nm_short, B.sesn_nm_eng)                         as year_season,
        ...
        SUM(SUM(A.sale_qty)) OVER (PARTITION BY (concat(B.year_nm_short, B.sesn_nm_eng), case when {{sel_category.value}} = 89 then 89 else coalesce(C.item_md_category_id, 91) end ) ORDER BY A.sale_dt)                AS cumulative_sale_qty,
        sum(A.sale_amt)                                                as sale_amt,
        SUM(SUM(A.sale_amt)) OVER (PARTITION BY (concat(B.year_nm_short, B.sesn_nm_eng), case when {{sel_category.value}} = 89 then 89 else coalesce(C.item_md_category_id, 91) end  ) ORDER BY A.sale_dt)                 AS cumulative_sale_amt,
        sum(A.sale_qty*E.tag_prce)                                     as sale_tag_amt,
        SUM(SUM(A.sale_qty * E.tag_prce)) OVER (PARTITION BY (concat(B.year_nm_short, B.sesn_nm_eng), case when {{sel_category.value}} = 89 then 89 else coalesce(C.item_md_category_id, 91) end ) ORDER BY A.sale_dt)      AS cumulative_tag_sale_amt
    ...
    join FIRST_OUT_DAY
       ...
       and case when {{sel_category.value}} = 89 then 89 else coalesce(C.item_md_category_id, 91) end = FIRST_OUT_DAY.cat_id
    join FIRST_SALE_DAY
       on concat(B.year_nm_short, B.sesn_nm_eng) = FIRST_SALE_DAY.year_season
       and case when {{sel_category.value}} = 89 then 89 else coalesce(C.item_md_category_id, 91) end = FIRST_SALE_DAY.cat_id
    join TOTAL_IN
       on concat(B.year_nm_short, B.sesn_nm_eng) = TOTAL_IN.year_season
       and case when {{sel_category.value}} = 89 then 89 else coalesce(C.item_md_category_id, 91) end = TOTAL_IN.cat_id
...
FROM DAY_DATA
WHERE ( ( cat_id = {{sel_category.value}} ) or ( {{sel_category.value}} = 89 ) )
