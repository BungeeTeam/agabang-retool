...
dim_style_color as(
    SELECT
      ...
      max(large_cat) as large_cat,
      max(middle_cat) as middle_cat,
      max(small_cat) as small_cat
    FROM dim_style as A
    WHERE CASE WHEN '{{ select_season_type.value }}' = 'yongpum' then substring(sty_cd,1,2) in ('{{ season_select_new.value.join("', '") }}')
         ELSE substring(sty_cd,1,2) = '01' END
    and CASE WHEN '{{ select_season_type.value }}' = 'yongpum' then substring(A.sty_cd,5,1) in ('6','8')
        ELSE substring(A.sty_cd,5,1) in ('1','2','3','5','7') END
...
filtered_daily_shop_sale_by_size as (
  SELECT *
  FROM agabang_dw.daily_shop_sales_by_size as A
  WHERE A.sale_dt >= toDate(addYears(now(), -5)) and A.sale_dt <= toDate(now()) 
    and CASE WHEN '{{ select_season_type.value }}' = 'yongpum' then substring(sty_cd,1,2) in ('{{ season_select_new.value.join("', '") }}')
             ELSE substring(sty_cd,1,2) = '01' END
    and CASE WHEN '{{ select_season_type.value }}' = 'yongpum' then substring(A.sty_cd,5,1) in ('6','8')
        ELSE substring(A.sty_cd,5,1) in ('1','2','3','5','7') END
    and CASE WHEN '{{ select_season_type.value }}' = 'yongpum' THEN true
...
FIRST_IN_DATE AS (
    ...
    WHERE cum_in_qty > 0
        and CASE WHEN '{{ select_season_type.value }}' = 'yongpum' then substring(sty_cd,1,2) in ('{{ season_select_new.value.join("', '") }}')
                 ELSE substring(sty_cd,1,2) = '01' END
        and CASE WHEN '{{ select_season_type.value }}' = 'yongpum' then substring(sty_cd,5,1) in ('6','8')
            ELSE substring(sty_cd,5,1) in ('1','2','3','5','7') END
        and CASE WHEN '{{ select_season_type.value }}' = 'yongpum' THEN true
...
SIZE_DATA_EACH_TOT AS (
  ...
  FROM filtered_daily_shop_sale_by_size
  group by 1,2,3,4
),
SIZE_DATA_EACH AS (
    select
...
WHERE 
concat(dim.sty_nm,' ',dim.sty_cd,' ',dim.fabric,' ',dim.col_nm,' ',dim.middle_cat,' ',dim.small_cat) like '%{{ search_text.value }}%'
  AND case when ({{sel_category.value ?? 0}} = 91) then D.item_md_category_id is null 
           when ({{sel_category.value ?? 0}} = 55) then true -- 전체:55
           when ({{sel_category.value ?? 0}} = 61) then substring(dim.sty_cd,5,2) in ('61','81') -- 용품: 발육
           when ({{sel_category.value ?? 0}} = 62) then substring(dim.sty_cd,5,2) in ('62','82') -- 용품: 발육
           when ({{sel_category.value ?? 0}} = 63) then substring(dim.sty_cd,5,2) in ('63','83') -- 용품: 발육
           when ({{sel_category.value ?? 0}} = 64) then substring(dim.sty_cd,5,2) in ('64','84') -- 용품: 발육
           when ({{sel_category.value ?? 0}} = 65) then substring(dim.sty_cd,5,2) in ('65','85') -- 용품: 발육
           when ({{sel_category.value ?? 0}} = 66) then substring(dim.sty_cd,5,2) in ('66','86') -- 용품: 발육
           when ({{sel_category.value ?? 0}} = 68) then substring(dim.sty_cd,5,2) in ('68','88') -- 용품: 발육
