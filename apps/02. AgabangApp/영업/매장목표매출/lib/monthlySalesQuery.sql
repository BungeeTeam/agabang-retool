...
SELECT 
  ...
  coalesce(yr, YEAR(year_month)) as yr,
  coalesce(month, MONTH(year_month)) as month,
...
FULL OUTER JOIN (
  ...
  WHERE date_format(year_month, '%m-%d') <= date_format(CURDATE(), '%m-%d')
) as B
ON A.biz_cd = B.biz_cd AND A.br_cd = B.br_cd AND A.shop_cd = B.shop_cd AND A.ym = date_format(B.year_month, '%y-%m')
)
SELECT 
  time_unit, yr, month, shop_cd, shop_nm, biz_cd, biz_nm,
  br_cd, br_nm,
...
FROM tbl
GROUP BY time_unit, yr, month, shop_cd, shop_nm, biz_cd, biz_nm, br_cd, br_nm, tp_cd, tp_nm, team_cd, team_nm, user_cd, user_nm, area_cd, area_nm, onoff_flag;