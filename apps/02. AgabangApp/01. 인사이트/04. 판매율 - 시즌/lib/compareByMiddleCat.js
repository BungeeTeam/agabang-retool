...
const data = {{ salesByMiddleCat.value}}; // 혹은 salesByLargeCat.value 등 원본 데이터
...
const dataByCategory = enrichedData.reduce((acc, item) => {
  // ▼▼▼▼▼ [수정] 그룹핑 키를 복합 키로 변경 ▼▼▼▼▼
  const compositeKey = `${item.large_cat}-${item.middle_cat}`;
...
    large_cat: thisYear.large_cat || lastYear.large_cat,
    middle_cat: thisYear.middle_cat || lastYear.middle_cat, // middle_cat 추가
...
return differenceData.sort((a, b) => {
