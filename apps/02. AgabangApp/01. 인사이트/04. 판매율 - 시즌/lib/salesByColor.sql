...
saleTbl as (
    SELECT
        ...
        coalesce(B.large_cat,'미분류') as large_cat, 
    coalesce(B.middle_cat,'미분류') as middle_cat, 
    coalesce(B.small_cat,'미분류') as small_cat,
        B.it as it, B.it_gb as it_gb, B.item as item,
        '총계' as total,
        CASE WHEN B.it = '5' THEN '기획' ELSE '정상' END as cat_group,
        CASE WHEN B.it in('2','3') THEN '시즌의류' ELSE coalesce(B.large_cat,'미분류') END as apparel_group,
