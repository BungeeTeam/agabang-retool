...
>
  <Body>
    <Text id="containerTitle2" value="#### 판매현황" verticalAlign="center" />
    <Spacer id="spacer13" />
    <Select
      id="select_item_type"
      captionByIndex=""
...
"
      ...
      <Event
        ...
        method="trigger"
        params={{ ordered: [] }}
        pluginId="get_item_list_new_fixed"
        type="datasource"
        waitMs="0"
        waitType="debounce"
      />
  ...
  : select_item_type.value === 'season_less' ? item.year_nm+'('+item.year_cd+')' 
  : select_item_type.value === 'mart' ? item.year_sesn_nm_eng +'(시즌코드: '+item.sesn_cd+')'
