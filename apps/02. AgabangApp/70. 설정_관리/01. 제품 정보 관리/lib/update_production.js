...
const getRowIdentifier = (row) => {
  ...
  const styNmDisplay = row['스타일명'] ? String(row['스타일명']).trim() : '';
  
  if (styNm) {
    return styNmDisplay ? `품번: ${styNm} (${styNmDisplay})` : `품번: ${styNm}`;
  } else {
    return styNmDisplay ? `스타일명: ${styNmDisplay}` : '품번 없음';
  }
};
  ...
  data.forEach(row => {
    ...
    if(row){
      ...
      for (const [excelField, dbField] of Object.entries(mappings)) {
        ...
        if ( ( excelField === 'MD제안납기' || excelField === '확정납기' ) && value) {
        ...
        // '수량' 의 경우 콤마를 제거합니다.
        if((excelField=='기획량'||excelField=='기획량2'||excelField=='기획량3'||excelField=='기획량4'|| excelField=='기획량5'||excelField=='예상TAG가(원)'||excelField=='확정TAG가(원)')
            && value ) {
          if(value === '-'){
            value = null;
          }
...
//      // 각 매핑에 대해 값을 처리합니다.
//      for (const [excelField, dbField] of Object.entries(mappings)) {
//        //  console.log(`excelField: ${excelField}, row: ${JSON.stringify(row)}`)
//        let value = row[excelField] ? String(row[excelField])?.trim() : null;
...
//        // '수량' 의 경우 콤마를 제거합니다.
//        if((excelField=='기획량'||excelField=='기획량2'||excelField=='기획량3'||excelField=='기획량4'|| excelField=='기획량5'||excelField=='예상TAG가(원)'||excelField=='확정TAG가(원)'||excelField=='확정TAG가(원)')
//            && value ) {
//          if(value === '-'){
//            value = null;
//          }
...
//        // 값이 있는 경우 작은따옴표를 이스케이프합니다. + 하이픈 제거 추가
//        if (value !== null && value !== undefined) {
//          //  value = value.replace(/-/g, ""); //하이픈 제거 추가
//          value = value.replace(/'/g, "''");
//          value = `'${value}'`;
//        } else {
//          value = 'NULL';
