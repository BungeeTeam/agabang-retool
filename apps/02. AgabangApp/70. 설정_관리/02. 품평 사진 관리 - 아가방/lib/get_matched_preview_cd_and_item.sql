...
preview_mapping AS (
    ...
    LEFT JOIN (
        SELECT DISTINCT sty_cd, it_gb
        FROM agabang_dw.dim_style
        WHERE comp_cd = 'AG001'
    ) ds ON rim.sty_cd = ds.sty_cd
    ...
    WHERE rim.br_cd = '{{ var_brand_code.value }}'
      ...
      OR (season_mapping.folder_season = 'SS' AND rim.sesn_cd IN ('1', '2', '3', '4'))
      OR (season_mapping.folder_season = 'FW' AND rim.sesn_cd IN ('5', '6', '7', '8'))
)
      AND (
        (season_mapping.folder_category = '언더' AND ds.it_gb IN ('11', '17', '51', '57'))
        OR (season_mapping.folder_category = '의류' AND ds.it_gb NOT IN ('11', '17', '51', '57'))
        OR (season_mapping.folder_category NOT IN ('언더', '의류'))
      )
),
final_mapping AS (
