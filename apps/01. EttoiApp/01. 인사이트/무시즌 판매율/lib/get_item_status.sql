WITH
  ...
  substring('{{ var_clicked_middle_cat.value?.year_cd }}',2,1)  AS selected_season_cd,
...
base_tbl AS (
  ...
  FROM (
    ...
    WHERE
      ...
      AND CASE WHEN selected_cat_id = '999' THEN true
               WHEN selected_cat_id = '666' THEN A.item_md_category_id in ('6','7','10')
  ...
  LEFT JOIN (
    ...
    WHERE br_cd = selected_br_cd
      ...
      AND it != '9'
      AND case when selected_cat_id = 666 then (it in ('6','7','10'))
               when selected_cat_id = 777 then (it in ('1','2','3','6','7'))
               when selected_cat_id = 888 then (it in ('4','5'))
               when selected_cat_id = 999 then (it in ('1','2','3','4','5','6','7','10'))
               else selected_cat_id = cast(it as int) end 
