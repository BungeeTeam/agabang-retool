...
-- 변수
    ...
    category_inout AS (
        ...
        -- CASE WHEN max(A.item_md_category_id) = '56' THEN '시즌언더'
        --      WHEN max(A.item_md_category_id) = '57' THEN '시즌의류'
        --      WHEN max(A.item_md_category_id) = '57' THEN '시즌용품'
        --      ELSE max(A.large_cat) END as cat_nm,

      -- CASE WHEN A.cat_id = '1' THEN '기초복'
      --      WHEN A.cat_id = '2' THEN '유아복'
      --      WHEN A.cat_id = '3' THEN '토들러'
      --      WHEN A.cat_id = '4' THEN '기획외의'
      --      WHEN A.cat_id = '5' THEN '기획내의'
      --      WHEN A.cat_id = '6' THEN '기초ACC'
      --      WHEN A.cat_id = '7' THEN '정상ACC'
      --      WHEN A.cat_id = '8' THEN '일반용품'
      --      WHEN A.cat_id = '9' THEN '임시'
      --      WHEN A.cat_id = '10' THEN '토들러ACC'
      --      WHEN A.cat_id = '12' THEN '출산용품'
      --      WHEN A.cat_id = '18' THEN '미분류'
      --      WHEN A.cat_id = '19' THEN '전체'
      --       ELSE A.cat_nm END as cat_nm,

      -- CASE WHEN max(A.item_md_category_id) = '1' THEN '기초복'
      --        WHEN max(A.item_md_category_id) = '2' THEN '유아복'
      --       WHEN max(A.item_md_category_id) = '3' THEN '토들러'
      --     WHEN max(A.item_md_category_id) = '4' THEN '기획외의'
      -- WHEN max(A.item_md_category_id) = '5' THEN '기획내의'
      -- WHEN max(A.item_md_category_id) = '6' THEN '기초ACC'
      -- WHEN max(A.item_md_category_id) = '7' THEN '정상ACC'
      -- WHEN max(A.item_md_category_id) = '8' THEN '일반용품'
      -- WHEN max(A.item_md_category_id) = '9' THEN '임시'
      -- WHEN max(A.item_md_category_id) = '10' THEN '토들러ACC'
      -- WHEN max(A.item_md_category_id) = '12' THEN '출산용품'
      -- WHEN max(A.item_md_category_id) = '18' THEN '미분류'
      -- WHEN max(A.item_md_category_id) = '19' THEN '전체'
      --        ELSE max(A.large_cat) END as cat_nm,



      -- CASE WHEN max(A.item_md_category_id) = '1' THEN '1'
      ...
      --        ELSE max(A.item_md_category_id) END as cat_id,
        ...
        CASE WHEN substring(max(A.sty_cd),5,1) in ('1','2','3','7') THEN 1
             WHEN substring(max(A.sty_cd),5,1) = '5' THEN 2
             ELSE 3 END as ttl_index
...
--         sum(A.sale_qty * B.tag_price)/sum(C.tot_in_amt) as this_week_progress_rate
        FROM agabang_dw.daily_sales_by_color AS A
        JOIN filtered_dim AS B 
          ON A.sty_cd = B.sty_cd 
            and A.col_cd = B.col_cd
        LEFT JOIN fitered_cum_sales AS C 
      ...
      A.cat_nm as cat_nm,
        ...
        LEFT JOIN category_monthly as B 
          ON A.cat_id = B.cat_id
        LEFT JOIN category_last_week as C 
          ON A.cat_id = C.cat_id
        LEFT JOIN category_this_week as D 
