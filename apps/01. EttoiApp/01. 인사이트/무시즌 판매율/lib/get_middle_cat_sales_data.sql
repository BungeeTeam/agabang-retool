...
-- 변수
    ...
    category_inout AS (
        SELECT
        -- CASE WHEN max(A.item_md_category_id) = '21' THEN '시즌의류(B)'
        --     WHEN max(A.item_md_category_id) = '22' THEN '시즌의류(T)'
        --     ELSE max(A.large_cat) END as cat_nm,
        max(A.large_cat) as cat_nm,
        max(A.middle_cat) as middle_cat,
        A.small_cat as small_cat,
        ...
        GROUP BY A.item_md_category_id, A.small_cat
        ORDER BY 1),
    ...
    category_monthly AS(
        ...
        max(B.middle_cat) as middle_cat,
        ...
        GROUP BY B.item_md_category_id, B.small_cat
        ORDER BY 1
    ),
...
--         sum(A.sale_qty * B.tag_price)/sum(C.tot_in_amt) as this_week_progress_rate
   ...
   SELECT
        -- CASE WHEN A.cat_id = '21' THEN '시즌의류(B)'
        --     WHEN A.cat_id = '22' THEN '시즌의류(T)'
        --     ELSE A.cat_nm END as cat_nm,
        A.cat_nm  as cat_nm,
        A.middle_cat as middle_cat,
        A.small_cat as small_cat,
        ...
        LEFT JOIN category_monthly as B ON A.cat_id = B.cat_id and A.middle_cat = B.middle_cat and A.small_cat = B.small_cat
        LEFT JOIN category_last_week as C ON A.cat_id = C.cat_id and A.middle_cat = C.middle_cat and A.small_cat = C.small_cat
        LEFT JOIN category_this_week as D ON A.cat_id = D.cat_id and A.middle_cat = D.middle_cat and A.small_cat = D.small_cat
...
--         --     WHEN max(A.item_md_category_id) = '22' THEN '시즌의류(T)'
--         --     ELSE max(A.large_cat) END as cat_nm,
--         -- CASE WHEN (max(A.item_md_category_id) = '21' or max(A.item_md_category_id) = '22') THEN '시즌의류'
--         --     ELSE max(A.large_cat) END as cat_nm,
--       -- CASE WHEN max(A.item_md_category_id) = '20' or max(A.item_md_category_id) = '56' THEN '시즌언더'
--       --      WHEN max(A.item_md_category_id) = '27' or max(A.item_md_category_id) = '62' THEN '시즌용품'
--       --      WHEN max(A.item_md_category_id) = '21' or max(A.item_md_category_id) = '22'  or max(A.item_md_category_id) = '57' THEN '시즌의류'
--       --      ELSE max(A.large_cat) END as cat_nm,

--       -- CASE WHEN max(A.item_md_category_id) = '20' or max(A.item_md_category_id) = '56' THEN '56'
--       --      WHEN max(A.item_md_category_id) = '27' or max(A.item_md_category_id) = '62' THEN '62'
--       --      WHEN max(A.item_md_category_id) = '21' or max(A.item_md_category_id) = '22'  or max(A.item_md_category_id) = '57' THEN '57'
--       --      ELSE max(A.item_md_category_id) END as cat_id,
...
--         WHERE A.br_cd = brand_code
--             and substring(A.sty_cd,3,2) = season_code
--         GROUP BY A.item_md_category_id, A.middle_cat
--         ORDER BY 1),
...
--             and MONTH(toDate(A.sale_dt)) = MONTH(deadline_date)
--             and A.sale_dt <= deadline_date
--         GROUP BY B.item_md_category_id, B.middle_cat
--         ORDER BY 1
--     ),
...
--         -- CASE WHEN A.cat_id = '21' THEN '시즌의류(B)'
--         --     WHEN A.cat_id = '22' THEN '시즌의류(T)'
--         -- CASE WHEN (A.cat_id = '21' or A.cat_id = '22') THEN '시즌의류'
--         --     ELSE A.cat_nm END as cat_nm,
--         -- A.middle_cat as middle_cat,
--         -- CAST(A.cat_id as INT) as cat_id,
--       -- CASE WHEN A.cat_id = '20' or A.cat_id = '56' THEN '시즌언더'
--       --      WHEN A.cat_id = '27' or A.cat_id = '62' THEN '시즌용품'
--       --      WHEN A.cat_id = '21' or A.cat_id = '22'  or A.cat_id = '57' THEN '시즌의류'
--       --      ELSE A.cat_nm END as cat_nm,

--       -- CASE WHEN A.cat_id = '20' or A.cat_id = '56' THEN 56
--       --      WHEN A.cat_id = '27' or A.cat_id = '62' THEN 62
--       --      WHEN A.cat_id = '21' or A.cat_id = '22'  or A.cat_id = '57' THEN 57
--       --      ELSE cast(A.cat_id as int) END as cat_id,
...
--         coalesce(D.this_week_sale_tag,0) /A.tot_in_tag as this_week_progress_rate
--         FROM category_inout as A
--         LEFT JOIN category_monthly as B ON A.cat_id = B.cat_id and A.middle_cat = B.middle_cat
--         LEFT JOIN category_last_week as C ON A.cat_id = C.cat_id and A.middle_cat = C.middle_cat
--         LEFT JOIN category_this_week as D ON A.cat_id = D.cat_id and A.middle_cat = D.middle_cat
-- --         JOIN category_cum as F ON A.cat_id = F.cat_id
--         WHERE A.cat_id is not null
-- ORDER BY A.ttl_index asc, A.cat_id asc