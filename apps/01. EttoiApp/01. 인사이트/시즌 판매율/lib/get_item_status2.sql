WITH
  ...
  -- '{{ moment(`${season_select.selectedItem.year_nm}-01-01`).subtract(2, 'years').format('YYYY-MM-DD') }}' as start_dt,
-- '{{ date_select.value }}' as end_dt,
-- '{{ season_select.selectedItem.year_cd }}' AS selected_yr_cd,
  '{{ var_clicked_middle_cat.value?.start_dt }}' as start_dt,
  '{{ var_clicked_middle_cat.value?.end_dt }}' as end_dt,
  -- '79' as selected_br_cd,
  '{{var_brand_code.value}}' as selected_br_cd,
  substring('{{ var_clicked_middle_cat.value?.year_cd }}',1,1) AS selected_yr_cd,
  '{{ season_select.selectedItem.sesn_cd }}' AS selected_season_cd,
  '{{ var_clicked_middle_cat.value?.cat_id }}' AS selected_cat_id,
  '{{ var_clicked_middle_cat.value?.middle_cat }}' AS selected_middle_cat,
    '{{ var_clicked_middle_cat.value?.small_cat }}' AS selected_small_cat,
...
base_tbl AS (
  ...
  FROM (
    ...
    LEFT JOIN agabang_dw.dim_style as A ON agabang_dw.daily_data_by_size.sty_cd = A.sty_cd and agabang_dw.daily_data_by_size.col_cd = A.col_cd and agabang_dw.daily_data_by_size.size_cd = A.size_cd
    WHERE
      ...
      AND CASE WHEN selected_cat_id = '999' THEN true
          WHEN selected_cat_id = '666' THEN A.item_md_category_id in ('6','7')
          WHEN selected_cat_id = '777' THEN A.item_md_category_id in ('1','2','3','6','7')
          WHEN selected_cat_id = '888' THEN A.item_md_category_id in ('4','5')
          ELSE A.item_md_category_id = selected_cat_id END
      AND A.middle_cat = selected_middle_cat
      AND A.small_cat = selected_small_cat
  ...
  LEFT JOIN (
    ...
    WHERE br_cd = selected_br_cd
      ...
      AND it != '9'
      AND case when selected_cat_id = 1 then it = '1'
               when selected_cat_id = 2 then it = '2'
               when selected_cat_id = 3  then it = '3'
               -- when selected_cat_id = 59 then it = '4'
               when selected_cat_id = 4 then it = '5'
               when selected_cat_id = 5 then it = '5'
               -- when selected_cat_id = 26 then it = '6'
               when selected_cat_id = 6 then it = '7'
               when selected_cat_id = 7 then it = '7'
               when selected_cat_id = 666 then it = '7'
