...
function exportReorderDataToExcel() {
  ...
  try {
    ...
    console.log("리오더 검토 테이블 생성 완료:", reviewTableData.length, "개");
    ...
    reorderData.forEach(item => {
      ...
      // 모든 사이즈에 대해 값이 없으면 0으로 설정
      allSizes.forEach(size => {
        if (!rowData[size]) rowData[size] = 0;
      });

      approvalTableData.push(rowData);
    });
    console.log("상신용 아소트 테이블 생성 완료:", approvalTableData.length, "개");
    ...
    console.log("메인 테이블 데이터 생성 완료:", mainTableData.length, "개");
    ...
    console.log("사이즈 테이블 데이터 생성 완료:", sizeTableData.length, "개");
