...
DAY_DATA as (
    select concat(B.year_nm_short, B.sesn_nm_eng)                         as year_season,
        ...
        SUM(SUM(A.sale_qty)) OVER (PARTITION BY (concat(B.year_nm_short, B.sesn_nm_eng), case when {{sel_category.value}} = 19 then 19 else coalesce(C.item_md_category_id, 18) end ) ORDER BY A.sale_dt)                AS cumulative_sale_qty,
        sum(A.sale_amt)                                                as sale_amt,
        SUM(SUM(A.sale_amt)) OVER (PARTITION BY (concat(B.year_nm_short, B.sesn_nm_eng), case when {{sel_category.value}} = 19 then 19 else coalesce(C.item_md_category_id, 18) end  ) ORDER BY A.sale_dt)                 AS cumulative_sale_amt,
        sum(A.sale_qty*E.tag_prce)                                     as sale_tag_amt,
        SUM(SUM(A.sale_qty * E.tag_prce)) OVER (PARTITION BY (concat(B.year_nm_short, B.sesn_nm_eng), case when {{sel_category.value}} = 19 then 19 else coalesce(C.item_md_category_id, 18) end ) ORDER BY A.sale_dt)      AS cumulative_tag_sale_amt
    ...
    join FIRST_OUT_DAY
       ...
       and case when {{sel_category.value}} = 19 then 19 else coalesce(C.item_md_category_id, 18) end = FIRST_OUT_DAY.cat_id
    join FIRST_SALE_DAY
    ...
    join TOTAL_IN
       ...
       and case when {{sel_category.value}} = 19 then 19 else coalesce(C.item_md_category_id, 18) end = TOTAL_IN.cat_id
