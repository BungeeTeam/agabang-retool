...
return data.error"
    ...
    />
    <SqlQuery
      id="get_cum_sales_by_week_backup"
      enableTransformer={true}
      errorTransformer="// The variable 'data' allows you to reference the request's data in the transformer. 
// example: return data.find(element => element.isError)
return data.error"
      isMultiplayerEdited={false}
      notificationDuration={4.5}
      query={include("./lib/get_cum_sales_by_week_backup.sql", "string")}
      resourceDisplayName="clickhouse-dw"
      resourceName="46922e5d-5645-4057-8fc8-3cc8cb9fbfe4"
  ...
  data.forEach(item => {
    const yearSeason = item.year_season;
    const weeks = item.weeks;  // 주차
    const dateRange = item.date_range; // 날짜 범위
    
    // 지표별 값을 추출합니다.
    const sale_qty_per = item.sale_qty_per;
    const sale_amt_per = item.sale_amt_per;
    const sale_tag_amt_per = item.sale_tag_amt_per;
    
    if (!groupedData[yearSeason]) {
      groupedData[yearSeason] = {
        x: [],
        customdata: [], // 날짜 범위를 저장할 배열
        sale_qty_per: [],
        sale_amt_per: [],
        sale_tag_amt_per: []
      };
    ...
    groupedData[yearSeason].x.push(weeks); // x축 데이터 (주차)
    groupedData[yearSeason].customdata.push(dateRange); // 날짜 범위 저장
    groupedData[yearSeason].sale_qty_per.push(sale_qty_per);
    groupedData[yearSeason].sale_amt_per.push(sale_amt_per);
    groupedData[yearSeason].sale_tag_amt_per.push(sale_tag_amt_per);
  });
  ...
  metrics.forEach(metric => {
    yearSeasons.forEach((yearSeason, index) => {
      ...
      const series = {
        name: yearSeason,
        x: groupedData[yearSeason].x,
        y: groupedData[yearSeason][metric],
        customdata: groupedData[yearSeason].customdata,
        type: 'line',
        mode: 'lines',
        hovertemplate: `
<b>시즌: %{fullData.name}</b><br>
<b>주차: %{x}주 (%{customdata})</b><br>
<b>${yAxisTitle}: ${hoverFormat}</b><extra></extra>
        ...
        transforms: [
          {
            type: 'sort',
            target: groupedData[yearSeason].x,
            order: 'ascending'
          }
        ]
      };
...
return data.error"
  ...
  const transform_data = formatDataAsArray(data).filter(
  i => {{select_item_type.value != 'yongpum'}} || {{select_brand.value}}.includes(i.sub_br_nm)
);
...
return data.error"
  ...
  <SqlQuery
    ...
    showSuccessToaster={false}
    transformer="return formatDataAsArray(data).map(i=>i.year_season_eng)"
    warningCodes={[]}
  >
    <Event
      event="success"
