...
STY_SEASON AS (
  ...
  and cast(A.br_cd as int) = {{ var_brand_code.value }}
),
SALE_DATA AS (
    select
    ...
    and cast(br_cd as int) = {{ var_brand_code.value }}
    group by 1,2,3
)
SELECT
    A.sty_cd AS sty_cd,
...
WHERE ( H.year_season_cd = '{{ select_season.value }}' OR '{{ select_season.value }}' = '' )
