...
return data"
  ...
  />
  <SqlQueryUnified
    id="update_category_memo"
    confirmationMessage="메모를 저장하시겠습니까?"
    errorTransformer="// The variable 'data' allows you to reference the request's data in the transformer. 
// example: return data.find(element => element.isError)
return data.error"
    isMultiplayerEdited={false}
    notificationDuration={4.5}
    query={include("./lib/update_category_memo.sql", "string")}
    requireConfirmation={true}
    resourceDisplayName="retool_db"
    resourceName="33c51bac-e1f2-4560-8260-3be760a1fd8f"
    runWhenModelUpdates={false}
...
return data.error"
    isMultiplayerEdited={false}
    notificationDuration={4.5}
    query={include("./lib/get_category_memo.sql", "string")}
    resourceDisplayName="retool_db"
    resourceName="33c51bac-e1f2-4560-8260-3be760a1fd8f"
    showSuccessToaster={false}
    showUpdateSetValueDynamicallyToggle={false}
  ...
  const transform_data = formatDataAsArray(data)
  transform_data?.forEach(item => {
    const { year_season, category_name, memo } = item;
  
    if (!result[year_season]) {
      result[year_season] = {};
    }

    result[year_season][category_name] = memo
  });
...
return transform(data)"
  ...
  <State
    ...
    persistValue={false}
    value={
      '["블루독베이비", "밍크뮤", "압소바", "타티네 쇼콜라", "해피랜드", "베베드피노", "베베드피노 언더웨어", "모이몰른", "모이몰른 US", "에뜨와"]'
    }
  />
  <JavascriptQuery
    id="set_brand_list"
