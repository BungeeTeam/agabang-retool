// 매장별 매출 현황 테이블 데이터를 엑셀 다운로드용으로 변환
// 금액 컬럼은 원단위로 변환 (백만원 * 1000000)
// salesByShop.value를 소스 데이터로 사용
// 매장코드 → 매장명 순서로 출력

utils.changeLocale("ko")

const data = salesByShop.value
const currentYear = thisYear.value
const lastYear = currentYear - 1
const yearBeforeLast = currentYear - 2

// 파일명 생성
const fileName = revDetailByShopModal.value.slice(4).replace("매출 상세", "").trim()
