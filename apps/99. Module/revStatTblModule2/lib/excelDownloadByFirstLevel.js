// 상위분류별 매출 현황 테이블 데이터를 엑셀 다운로드용으로 변환
// 금액 컬럼은 원단위로 변환 (백만원 * 1000000)
// salesByFirstLevel.value를 소스 데이터로 사용
// 주의: salesByFirstLevel은 target_sales가 없음

utils.changeLocale("ko")

const data = salesByFirstLevel.value.filter(item => item.sales_type === selectedSalesRow.value?.sales_type)
