// 차상위분류별 매출 현황 테이블 데이터를 엑셀 다운로드용으로 변환
// 금액 컬럼은 원단위로 변환 (백만원 * 1000000)
// salesBySecondLevel.value를 소스 데이터로 사용
// 주의: salesBySecondLevel은 target_sales가 없음

utils.changeLocale("ko")

const data = salesBySecondLevel.value.filter(item => item.sales_type === selectedSalesRow.value?.sales_type)
...
const exportData = data.map(row => {
  ...
  const newRow = {
    // 1. 상위분류
    "상위분류": row.first_lv_class || "",
    
    // 2. 차상위분류
    "차상위분류": row.second_lv_class || "",
...
// 파일명 생성
const fileName = `차상위 카테고리별 매출 현황`
