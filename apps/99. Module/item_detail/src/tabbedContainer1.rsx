      ...
      <View id="41e45" viewKey="View 1">
        ...
        >
          ...
          <Series
            ...
            connectorLineColor="#000000"
            dataLabelPosition="none"
            datasource="{{ JSON.parse(get_online_sales.data[2].sale_trend) }}"
            datasourceMode="manual"
            decreasingBorderColor="#000000"
            decreasingColor="#000000"
            filteredGroups={null}
            ...
            markerSize={6}
            markerSymbol="circle"
            name="{{ get_online_sales.data[2].sale_gb }}"
            showMarkers={false}
            textTemplateMode="source"
            type="line"
            waterfallBase={0}
            waterfallMeasures={{ array: [] }}
            waterfallMeasuresMode="source"
            xData="{{ formatDataAsObject(JSON.parse(get_online_sales.data[2].sale_trend)).day }}"
            xDataMode="source"
            yAxis="y"
            yData="{{ formatDataAsObject(JSON.parse(get_online_sales.data[2].sale_trend)).val }}"
            yDataMode="source"
            zData="[1, 2, 3, 4, 5]"
            zDataMode="manual"
          />
          <Series
            ...
            connectorLineColor="#000000"
            dataLabelPosition="none"
            datasource="{{ JSON.parse(get_online_sales.data[3].sale_trend) }}"
            datasourceMode="manual"
            decreasingBorderColor="#000000"
            decreasingColor="#000000"
            filteredGroups={null}
            ...
            markerSize={6}
            markerSymbol="circle"
            name="{{ get_online_sales.data[3].sale_gb }}"
            showMarkers={false}
            textTemplateMode="source"
            type="line"
            waterfallBase={0}
            waterfallMeasures={{ array: [] }}
            waterfallMeasuresMode="source"
            xData="{{ formatDataAsObject(JSON.parse(get_online_sales.data[3].sale_trend)).day }}"
            xDataMode="source"
            yAxis="y"
            yData="{{ formatDataAsObject(JSON.parse(get_online_sales.data[3].sale_trend)).val }}"
            yDataMode="source"
            zData="[1, 2, 3, 4, 5]"
            zDataMode="manual"
          />
    ...
    >
      ...
      <View id="41e45" viewKey="View 1">
        <Chart
          ...
          title="판매 비중"
          valueData={
            '{{\n  get_online_sales.data.map(item => {\n    const trend = JSON.parse(item.sale_trend || "[]");\n    const last = trend.at(-1);\n    return last ? last.val : 0;  // 마지막 값이 없으면 0으로 처리\n  })\n}}'
          }
        />
      </View>
    </Container>
