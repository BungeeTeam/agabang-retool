<App>
  ...
  <GlobalWidgetProp id="brand_code" description="브랜드 코드" value="'01'" />
  <GlobalWidgetOutput
    id="selected_sty_info"
    description="아이템 디테일을 위한 선택된 스타일 정보"
    value="{{ var_sel_sty_info.value }}"
  />
  ...
  >
    ...
    >
      ...
      <ListViewBeta
        id="listView1"
        data="{{ get_category_list_new.data[sel_category.value] }}"
        heightType="auto"
        itemWidth="200px"
        margin="0"
        numColumns={3}
              ...
              ordered: [
                {
                  0: {
                    ordered: [
                      ...
                      {
                        datasource:
                          "{{get_item_size_detail.data['tot_in_qty']}}",
                      },
                      { chartType: "pie" },
                      { aggregationType: "sum" },
                      { color: null },
