<GlobalFunctions>
  ...
  <Folder id="TeamsAlert">
    ...
    />
    <RESTQuery
      id="disney_TeamsTriggerForManuTeam"
      body="{{ prepareApiPayload.value }}"
      bodyType="raw"
      cookies={'[{"key":"","value":""}]'}
      headers={'[{"key":"Content-Type","value":"application/json"}]'}
      isMultiplayerEdited={false}
      resourceDisplayName="디즈니 생산팀 팀즈봇 트리거"
      resourceName="41462c0f-1f12-4f52-87c3-999762060da3"
      resourceTypeOverride=""
      runWhenModelUpdates={false}
      successMessage={
        "Teams \"클리브 프로젝트 - 번지 테스트 채널\"에 메시지를 발송했습니다. ({{ prepareApiPayload.value.items.length > 0 ? (prepareApiPayload.value.items.length === 1 ? prepareApiPayload.value.items[0].style_name : `${prepareApiPayload.value.items[0].style_name} 외 ${prepareApiPayload.value.items.length - 1}개`) : '' }})"
    ...
    />
    <JavascriptQuery
      id="getNotificationItems_backup"
      isMultiplayerEdited={false}
      notificationDuration={4.5}
      query={include("./lib/getNotificationItems_backup.js", "string")}
      resourceName="JavascriptQuery"
      runWhenPageLoadsDelay="30000"
      showSuccessToaster={false}
    />
    <Function
      id="prepareApiPayloadTest"
      funcBody={include("./lib/prepareApiPayloadTest.js", "string")}
    />
  </Folder>
