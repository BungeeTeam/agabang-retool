...
function getSelectedItems() {
  ...
  // 원본 데이터 가져오기
  const colorListData = getColorList.data || [];
  ...
  const analysisReport = selectedColorHeaderRows.map(item => {
    ...
    const sizeReorderInfo = sizeItems.map(sizeItem => {
      ...
      const sizeSalesRate = sizeSalesQty > 0 ? sizeSalesQty / sizeInQty : 0;
    ...
    return {
      ...
      sales_performance: {
        ...
        "일 판매량": safeNumber(item.sales_period_until_reference_date) > 0 ? 
            Number((safeNumber(item.tot_sale_qty) / safeNumber(item.sales_period_until_reference_date)).toFixed(1)) : 0,
        
        // 주간판매실적
        "매장수": safeNumber(shopCount),
        "최근 7일 판매량": safeNumber(item.weekly_7d_sale_qty),
        "최근 7일 판매율(출고 기준)": safeNumber(item.tot_out_qty) > 0 ? 
            safeNumber(item.weekly_7d_sale_qty) / safeNumber(item.tot_out_qty) : 0,
        "매장당 일 판매량": shopDailySales
        //  "매장수": safeInteger(shopCount),
        //  "점일 판매량": safeInteger(safeNumber(item.sales_period_until_reference_date) > 0 ? 
        ...
        //  "최근 7일 판매율(출고 기준)": Number((safeNumber(item.tot_out_qty) > 0 ? safeNumber(item.weekly_7d_sale_qty) / safeNumber(item.tot_out_qty) : 0).toFixed(4))
      ...
      reorder_info: {
        ...
        "목표 판매율(%)": safeInteger(targetSaleRate * 100),
        "MD 메모": '(메모 없음)',
