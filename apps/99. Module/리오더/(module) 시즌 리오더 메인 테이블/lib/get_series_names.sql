WITH item_styles AS (
  SELECT 
   ...
   FROM dim_style
    ...
    AND substring(sty_cd, 3, 1) = substring('{{ select_season.value}}', 1, 1)
    AND season_cd in (substring('{{ select_season.value}}', 2, 1))
    AND season_cd != '0'
),
...
cleaned_nm LIKE '%포인트%', REGEXP_REPLACE(cleaned_nm, '(.+?)포인트.*', '\\1'),
cleaned_nm LIKE '%조거핏%', REGEXP_REPLACE(cleaned_nm, '(.+?)조거핏.*', '\\1'),
cleaned_nm LIKE '%삼각%', REGEXP_REPLACE(cleaned_nm, '(.+?)삼각.*', '\\1'),
cleaned_nm LIKE '%패턴%', REGEXP_REPLACE(cleaned_nm, '(.+?)패턴.*', '\\1'),
...
cleaned_nm LIKE '%메쉬%', REGEXP_REPLACE(cleaned_nm, '(.+?)메쉬.*', '\\1'),
...
cleaned_nm LIKE '%6부%', REGEXP_REPLACE(cleaned_nm, '(.+?)6부.*', '\\1'),
cleaned_nm LIKE '%7부%', REGEXP_REPLACE(cleaned_nm, '(.+?)7부.*', '\\1'),
cleaned_nm LIKE '%칠부%', REGEXP_REPLACE(cleaned_nm, '(.+?)칠부.*', '\\1'),
...
cleaned_nm LIKE '%슬림%', REGEXP_REPLACE(cleaned_nm, '(.+?)슬림.*', '\\1'),
cleaned_nm LIKE '%오픈%', REGEXP_REPLACE(cleaned_nm, '(.+?)오픈.*', '\\1'),
cleaned_nm LIKE '%원피스%', REGEXP_REPLACE(cleaned_nm, '(.+?)원피스.*', '\\1'),
cleaned_nm LIKE '%우주복%', REGEXP_REPLACE(cleaned_nm, '(.+?)우주복.*', '\\1'),
cleaned_nm LIKE '%오버올%', REGEXP_REPLACE(cleaned_nm, '(.+?)오버올.*', '\\1'),
