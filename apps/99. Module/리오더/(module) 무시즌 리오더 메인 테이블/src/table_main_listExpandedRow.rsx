<ExpandedRow id="table_main_listExpandedRow">
  <Table
    ...
    data="{{ tempTableData.value.filter(item => 
   item._type === 'size' &&
...
전월 대비 증감도 함께 보여드려요."
  ...
  return `${year}년 ${month}월`;
    ...
    <Column
      ...
      key="week_1_sale_qty"
      label="{{ (() => {
  const now = new Date('2025-05-26T13:28:00+09:00'); // KST 기준 현재 시간 (실제로는 new Date())
...
전주 대비 증감율 = ((이번 주 판매량 - 전주 판매량) / 전주 판매량) * 100"
    ...
    <Column
      ...
      key="week_2_sale_qty"
      label="{{ (() => {
  const now = new Date('2025-05-26T12:08:00+09:00'); // KST 기준 현재 시간 (고정된 날짜로 테스트, 실제로는 new Date() 사용)
  ...
  return `${month}월 ${weekNumber}주차 (${formatDate(week2Start)}~${formatDate(week2End)})`;
    ...
    <Column
      ...
      key="week_3_sale_qty"
      label="{{ (() => {
  const now = new Date('2025-05-26T13:12:00+09:00'); // KST 기준 현재 시간 (실제로는 new Date())
  ...
  return `${month}월 ${weekNumber}주차 (${formatDate(week3Start)}~${formatDate(week3End)})`;
    ...
    <Column
      ...
      key="week_4_sale_qty"
      label="{{ (() => {
  const now = new Date('2025-05-26T13:12:00+09:00');
