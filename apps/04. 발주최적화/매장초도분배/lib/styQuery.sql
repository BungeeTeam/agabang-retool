...
from (
  ...
  from agabang.dsin
  where substr(sty_cd, 1, 2) = '{{ brcd.value }}' 
    and substr(sty_cd, 3, 2) = '{{ seasonSelect.value || "-" }}'
    and io_type = 'I'
    and wh_cd = '1000'
...
left join (
  ...
  where substr(sty_cd, 3, 2) = '{{ seasonSelect.value || "-" }}' 
) t2
...
-- from (
--   select
--     sty_cd, col_cd, size_cd, plan_qty, COALESCE(updt_dt, toDate(plan_date)) as plan_dt
