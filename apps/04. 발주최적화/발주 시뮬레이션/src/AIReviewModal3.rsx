  ...
  <Header>
    ...
    >
      <Event
        event="click"
        method="setValue"
        params={{ ordered: [{ value: "{{ !isHidden3.value}}" }] }}
        pluginId="isHidden3"
        type="state"
        waitMs="0"
        waitType="debounce"
          ...
          ordered: [
            {
              options: {
                ordered: [
                  {
                    message:
                      '{{ reviewPrompt3.value + "<data>" + JSON.stringify(simulTbl3.data) + "</data>"}} ',
                  },
