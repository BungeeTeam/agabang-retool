...
--             {{ varShopSalesCatogory.value?.category_name === '총계' ? `'총계'` :  `CASE WHEN substring(item,1,1) = '1' THEN '시즌언더'
...
--             AND substring(item,1,1) not in ('6', '8') AND B.onoff_flag in ({{tabs1.value==='합계' ? `'오프라인', '온라인'`:`'${tabs1.value}'`}})
--             AND toDate(wrk_dt) >= toDate(addYears(start_date, -3))
--         GROUP BY shop_cd, category_name
--     ),
...
--     WHEN substring(item,1,1) = '9' THEN '판촉부자재'
--   ELSE '기타' END` }} as category_name,
--           sum(CASE WHEN sale_dt BETWEEN toDate(start_date) AND toDate(end_date) THEN sales_qty ELSE 0 END) as cur_qty,
--           sum(CASE WHEN sale_dt BETWEEN toDate(addYears(end_date, -1)) AND toDate(end_date) THEN sales_qty ELSE 0 END) as cur_tot_qty,
--           sum(CASE WHEN sale_dt BETWEEN toDate(start_date) AND toDate(end_date) THEN sales_price ELSE 0 END) as cur_rev,
--           sum(CASE WHEN sale_dt BETWEEN toDate(addYears(end_date, -1)) AND toDate(end_date) THEN sales_price ELSE 0 END) as cur_tot_rev,
--           sum(CASE WHEN sale_dt BETWEEN toDate(start_date) AND toDate(end_date) THEN sales_qty*tag_price ELSE 0 END) as cur_tag,
--           sum(CASE WHEN sale_dt BETWEEN toDate(addYears(end_date, -1)) AND toDate(end_date) THEN sales_qty*tag_price ELSE 0 END) as cur_tot_tag,
--           sum(CASE WHEN sale_dt BETWEEN toDate(start_date) AND toDate(end_date) THEN cost_price * sales_qty ELSE 0 END) as cur_cost,
--           sum(CASE WHEN sale_dt BETWEEN toDate(addYears(start_date, -1)) AND toDate(addYears(end_date, -1)) THEN sales_qty ELSE 0 END) as prev_qty,
--           sum(CASE WHEN sale_dt <= toDate(addYears(end_date, -1)) THEN sales_qty ELSE 0 END) as prev_tot_qty,
--           sum(CASE WHEN sale_dt BETWEEN toDate(addYears(start_date, -1)) AND toDate(addYears(end_date, -1)) THEN sales_price ELSE 0 END) as prev_rev,
--           sum(CASE WHEN sale_dt <= toDate(addYears(end_date, -1)) THEN sales_price ELSE 0 END) as prev_tot_rev,
--           sum(CASE WHEN sale_dt BETWEEN toDate(addYears(start_date, -1)) AND toDate(addYears(end_date, -1)) THEN sales_qty*tag_price ELSE 0 END) as prev_tag,
--           sum(CASE WHEN sale_dt <= toDate(addYears(end_date, -1)) THEN sales_qty*tag_price ELSE 0 END) as prev_tot_tag,
--           sum(CASE WHEN sale_dt BETWEEN toDate(addYears(start_date, -1)) AND toDate(addYears(end_date, -1)) THEN cost_price * sales_qty ELSE 0 END) as prev_cost
--         FROM agabang_dw.daily_shop_sales_by_dimension
...
-- FROM AllSales as S
-- FULL OUTER JOIN AllOutRtn AS O ON S.shop_cd = O.shop_cd and S.category_name = O.category_name
-- WHERE S.category_name = '{{ varShopSalesSummary.value?.category_name }}'
-- ORDER BY shop_cd, category_name;