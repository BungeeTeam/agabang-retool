with rt_tbl as (
    select
      ...
      where ord_fix_yn = 'Y' and substring(sty_cd, 1, 2) = '{{ bizSelect.value }}' and toYear(toDate(ord_dt)) >= toYear(today()) - 2 and reg_tp != 'R'
      group by ord_dt, to_shop_cd, sty_cd, col_cd, size_cd, rt_fr_dt
),
sales_stat as (
    select
      rt_start_dt,
      coalesce(sum(if(A.rt_dt <= B.sale_dt AND A.rt_end_dt >= B.sale_dt, B.sales_qty, 0)), 0) AS tot_sales_qty,
      coalesce(sum(if(A.rt_dt <= B.sale_dt AND A.rt_end_dt >= B.sale_dt, B.sales_price, 0)), 0) AS rt_rev
    from rt_tbl as A
    left join (
      select
...
select
    ...
    rt_sty_cnt,
    rt_ord_qty,
    coalesce(tot_sales_qty, 0) as tot_sales_qty,
    rt_rev,
    coalesce(tot_sales_qty, 0) / rt_ord_qty as sales_ratio,
