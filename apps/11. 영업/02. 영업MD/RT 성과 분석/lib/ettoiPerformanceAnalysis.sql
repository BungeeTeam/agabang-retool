with rt_tbl as (
    select
        ...
        case 
          when ord_id = 'AUTO07' or (reg_tp = 'E' and rt_start_dt >= '2025-05-13') then 'Cleave'
          when ord_id != 'AUTO07' and reg_tp = 'A' then 'Auto'
          else 'RequestByShop'
        end as ord_id,
    ...
    where ord_fix_yn = 'Y' 
  and date_format(toDate(ord_dt), '%m-%d') >= date_format(DATE '2025-05-13', '%m-%d') 
  and toYear(toDate(ord_dt)) >= 2024
  and date_format(toDate(ord_dt), '%m-%d') <= date_format(subtractDays(today(), 40), '%m-%d')
  and fr_qty > 0
  and substring(sty_cd, 1, 2) = '07'
