...
select
    ...
    avg(tot_sales_qty / rt_ord_qty) OVER (PARTITION BY reg_tp ORDER BY rt_start_dt ROWS BETWEEN 7 PRECEDING AND CURRENT ROW) as ma_sales_ratio
from (
    select
        reg_tp, rt_start_dt, count(distinct(sty_cd)) as rt_sty_cnt, sum(rt_qty) as rt_ord_qty
    from rt_tbl
    group by reg_tp, rt_start_dt
) as t1
left join sales_stat as t2
on t1.rt_start_dt = t2.rt_start_dt and t1.reg_tp = t2.reg_tp
where rt_sty_cnt >= 3
;