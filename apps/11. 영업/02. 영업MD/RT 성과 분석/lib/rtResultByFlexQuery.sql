with rt_tbl as (
    ...
    from (
        select
            ...
            addDays(toDate(ord_dt), 30) as rt_end_dt,
            sty_cd,
            col_cd,
            size_cd,
        ...
        where ord_fix_yn = 'Y' and substring(sty_cd, 1, 2) = '{{ bizSelect.value }}' and toYear(toDate(ord_dt)) >= toYear(today()) - 2 and fr_qty > 0
        group by reg_tp, ord_dt, to_shop_cd, sty_cd, col_cd, size_cd, rt_dt
    ) as t1
    left join (
        select distinct shop_cd, is_flex from agabang_dw.dim_shop
    ) as t2
...
select
    ...
    avg(tot_sales_qty / rt_ord_qty) OVER (PARTITION BY is_flex ORDER BY rt_start_dt ROWS BETWEEN 7 PRECEDING AND CURRENT ROW) as ma_sales_ratio
from (
    select
        is_flex, rt_start_dt, count(distinct(sty_cd)) as rt_sty_cnt, sum(rt_qty) as rt_ord_qty
    from rt_tbl
    group by is_flex, rt_start_dt
) as t1
left join sales_stat as t2
on t1.rt_start_dt = t2.rt_start_dt and t1.is_flex = t2.is_flex
where rt_sty_cnt >= 3
;