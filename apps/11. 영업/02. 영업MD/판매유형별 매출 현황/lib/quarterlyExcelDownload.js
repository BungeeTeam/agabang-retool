// 분기별 테이블 데이터를 엑셀 다운로드용으로 변환
// 금액 컬럼은 원단위로, 퍼센트 컬럼은 퍼센트로 변환

utils.changeLocale("ko")

const data = quarterly.value
const headers = tableHeader.value

// 1분기부터 4분기 순서로 정렬
const sortedData = [...data].sort((a, b) => {
  // quarter_unit 기준 오름차순
  const quarterCompare = a.quarter_unit - b.quarter_unit
  if (quarterCompare !== 0) return quarterCompare
  
  // 같은 분기 내에서는 sales_type 내림차순 -> season_cd 오름차순
...
const exportData = sortedData.map(row => {
  ...
  // 분기 컬럼 (맨 앞)
  newRow["분기"] = row.quarter_unit ? `${row.quarter_unit}` : ''
...
// 파일명 생성
const fileName = `(${dateRange.value.end} 마감) 분기별 정상_이월 판매데이터`
