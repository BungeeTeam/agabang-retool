...
const mappingKeys = ["season_nm","quarter_unit","month_unit", "sales_type"];

// target_new를 기준으로 Map 생성
const targetMap = new Map(
  target_new.map(item => {
    const key = mappingKeys.map(k => item[k]).join("|");
    return [key, item.target_sales];
  })
);
console.log(allGroups)

// result의 각 항목에 target_sales 병합
...
const subTotalKeys = ['sales_type','month_unit']

const TotalKeys = ['month_unit']


const subTotal = subTotals(allGroups, subTotalKeys, ["target_sales", ...sumKeys]);
const grandTotal = total(allGroups,TotalKeys,["target_sales", ...sumKeys])
