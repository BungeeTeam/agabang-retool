...
const data = {{ getSalesByShop.data}}
  ...
  .filter(i => !{{ selectedRow_old.value?.item_nm }} || i.item_nm === {{ selectedRow_old.value?.item_nm }})
  .filter(i => !{{ selectedRow_old.value?.br_nm }} || {{ selectedRow_old.value?.br_nm === '소계' }} || i.br_nm === {{ selectedRow_old.value?.br_nm }})
