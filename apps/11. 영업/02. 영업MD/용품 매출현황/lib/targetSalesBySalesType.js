const data = {{ formatDataAsArray(periodicTargetSales.data).filter(i=> tabsPeriod.value === '합계' ? true : i.onoff_flag===tabsOnOff.value)  }}
const target = {{ formatDataAsArray(targetRate.data).filter(i=> tabsPeriod.value === '합계' ? true : i.onoff_flag===tabsOnOff.value)  }}
