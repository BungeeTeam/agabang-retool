...
WHERE biz_cd in ({{ bizMultiSelect3.value.map(item => `'${item}'`).join(',') }}) 
{{ selectedRow2.value?.br_nm ? `and Style.br_nm = '${selectedRow2.value.br_nm}'` : '' }}
{{ selectedRow2.value?.it_gb_nm ? `and Style.it_gb_nm = '${selectedRow2.value.it_gb_nm}'` : '' }}
{{ selectedRow2.value?.item_nm ? `and Style.item_nm = '${selectedRow2.value.item_nm}'` : '' }}
{{ selectedRow2.value?.sub_br_nm ? `and Style.sub_br_nm = '${selectedRow2.value.sub_br_nm}'` : '' }}
and onoff_flag = '오프라인'
GROUP BY
    -- Shop.biz_cd, Shop.biz_nm,
