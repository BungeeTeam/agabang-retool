...
function addSaleRates(data) {
  return data.map(item => {
    ...
    const sale_tag_rate = item.out_amt > 0 
      ? (item.sale_tag / item.out_amt) 
      : 0;
    
    const sale_amt_rate = item.out_amt > 0 
      ? (item.sale_amt / item.out_amt) 
      : 0;
    ...
    const sale_tag_rate_ly = item.out_amt_ly > 0 
      ? (item.sale_tag_ly / item.out_amt_ly) 
      : 0;
    
    const sale_amt_rate_ly = item.out_amt_ly > 0 
      ? (item.sale_amt_ly / item.out_amt_ly) 
      : 0;
...
function addSubtotalRow(data, sumKeys, arrData) {
  ...
  subtotal.sale_tag_rate = subtotal.out_amt > 0 
    ? subtotal.sale_tag / subtotal.out_amt
    : 0;
  
  subtotal.sale_amt_rate = subtotal.out_amt > 0 
    ? subtotal.sale_amt / subtotal.out_amt
    : 0;
  ...
  subtotal.sale_tag_rate_ly = subtotal.out_amt_ly > 0 
    ? subtotal.sale_tag_ly / subtotal.out_amt_ly
    : 0;
  
  subtotal.sale_amt_rate_ly = subtotal.out_amt_ly > 0 
    ? subtotal.sale_amt_ly / subtotal.out_amt_ly
    : 0;
...
const sumKeys = ["sale_qty", "sale_tag", "sale_amt", "out_qty", "out_amt",
                "sale_qty_ly", "sale_tag_ly", "sale_amt_ly", "out_qty_ly", "out_amt_ly"]
...
const withSubtotal = addSubtotalRow(withGrowth, sumKeys, arrData)

// 매출 기여도 점수 계산 함수 (100점 만점)
function addContributionScore(data) {
  // 소계 데이터 찾기
  const totals = data.find(d => d.tp_cd === '소계');
  
  if (!totals || totals.shop_sale_count === 0 || totals.sale_amt === 0) {
    return data; // 계산 불가능한 경우 원본 반환
  }
  
  // 전체 평균 효율성 (매장당 평균 매출)
  const overallAvgSalesPerShop = totals.sale_amt / totals.shop_sale_count;
  ...
  return data.map(channel => {
    ...
    // 매출 기여도 계산
    // 1. 매출 비중
    const salesShare = channel.sale_amt / totals.sale_amt;
    
    // 2. 매장 비중
    const shopShare = channel.shop_sale_count / totals.shop_sale_count;
    
    // 3. 상대적 효율성 (매출 비중 / 매장 비중)
    const relativeEfficiency = shopShare > 0 ? salesShare / shopShare : 0;
    
    // 4. 100점 만점으로 변환
    // relativeEfficiency가 1이면 평균 수준 (50점)
    // 2배 효율이면 100점, 0.5배 효율이면 25점으로 스케일링
    let score;
    if (relativeEfficiency >= 2) {
      score = 100; // 상한선
    } else if (relativeEfficiency >= 1) {
      // 1~2 구간을 50~100점으로 매핑
      score = 50 + (relativeEfficiency - 1) * 50;
    } else {
      // 0~1 구간을 0~50점으로 매핑
      score = relativeEfficiency * 50;
    }
    
    return {
      ...channel,
      contribution_score: Math.round(score * 10) / 10 // 소수점 1자리까지 반올림
    };
  });
}

// 매출 기여도 점수 추가
const finalResult = addContributionScore(withSubtotal)

return finalResult.sort((a, b) => {
  // 소계가 항상 맨 아래로
  if (a.tp_cd === '소계') return 1;
  if (b.tp_cd === '소계') return -1;
  
  // 나머지는 contribution_score 기준으로 내림차순 정렬 (높은 점수가 위로)
