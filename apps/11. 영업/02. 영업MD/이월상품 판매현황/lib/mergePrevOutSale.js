const out = {{ varResult.value.prevOut }}
const sale = {{ varResult.value.prevSale }}

// 병합 기준 컬럼 정의
const mergeFields = [
  "biz_cd", "biz_nm", "br_cd", "br_nm", "shop_cd", "shop_nm",
...
const mergedData = Array.from(mergedKeys).map(key => {
  ...
  mergeFields.forEach(field => {
    base[field] = outItem?.[field] || saleItem?.[field] || "";
  });
...
const online_data = {{ processPrevOnlineWarehouse.value }};
